...
\begin{document}
	...
	\section{OP-TEE}
	\label{sec:optee}
	Open Portable Trusted Execution Environment \cite{optee} è un progetto open source nato nel 2013 per fornire un ambiente di esecuzione sicuro e affidabile per architetture ARM, sfruttando la tecnologia TrustZone, anche se è stato strutturato in modo tale da essere compatibile con altre tecnologie hardware che forniscono le funzionalità di un TEE.

	OP-TEE è stato sviluppato per essere compatibile con le specifiche GlobalPlatform, per questo motivo il software presenta caratteristiche architetturali simili.
	...
	% Da scrivere: 
	%	Riportare informazioni Passthrough
	%		Sottosistema Linux? Maybe qualche accenno?
	%		Sicuramente OP-TEE
	%		Sicuramente implementazione
	%	Modello dell'ambiente pensato
	%	Cosa si vuole fare e soluzioni pensate
