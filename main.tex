...
\begin{document}
	...
	SGX consiste in un insieme di istruzioni hardware e strutture di memoria che consentono la creazione e l'isolamento di \textit{enclavi} sicure all'interno del processore, ovvero piccole zone di memoria crittografate e protette tramite meccanismi hardware. Per l'accesso a queste zone di memoria è necessario utilizzare i nuovi metodi introdotti da SGX, come ad esempio l'istruzione \texttt{ECREATE} permette di creare un'istanza di enclave, mentre l'istruzione \texttt{ECALL} consente alle applicazioni di interagire con il codice all'interno dell'enclave in modo controllato e sicuro. 
	
	Le istruzioni che introduce sono comparabili a quelle per cambiare da user mode a kernel mode. Le enclavi vengono sempre eseguite al ring 3 di privilegio (il più basso) ed utilizzano la traduzione degli indirizzi fornita da kernel e hypervisor.
	...
	All'interno di questa zona di memoria si possono trovare la \textit{Enclave Page Cache} (EPC), ovvero pagine di 4KB contenenti codici e dati delle enclavi, oltre all'\textit{Enclave Page Cache Map} (EPCM), la quale serve alla CPU per tracciare lo stato delle pagine all'interno dell'EPC, permettendo di garantire che ciascuna pagina appartenga ad una sola enclave. Quest'ultima è sempre inaccessibile, indipendentemente dal livello di privilegio.
	...
	Per questo motivo AMD SEV permette di crittografare in modo granulare la memoria associata a una specifica macchina virtuale, conferendole un livello di inaccessibilità anche per l'hypervisor stesso. La cifratura dei dati in memoria è affidata ad un chip hardware AES-128. Comunque esiste la possibilità da parte della VM di condividere determinate pagine con l'hypervisor.
	
	In aggiunta, sulla scheda madre, trova posto un processore ARM denominato \textit{Secure Processor} (SP), con un suo firmware proprietario. Questo componente assume responsabilità diverse, tra cui spicca la gestione delle chiavi crittografiche, ognuna delle quali è unica per una singola VM. Questo processore fornisce delle API che l'hypervisor deve utilizzare per gestire le chiavi.
	Il SP permette anche un processo di attestazione remota, per aver modo di verificare che le VM in esecuzione siano effettivamente quelle desiderate, all'interno dell'ambiente corretto. Il report generato contiene tutte le informazioni necessarie per verificarlo e viene firmato tramite una chiave privata presente solamente all'interno del processore, la quale corrispondente pubblica viene fornita da AMD.
	...
	Al termine di questo stato la VM passa in modalità di esecuzione regolare.
